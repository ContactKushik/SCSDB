...
const Header = ({ data }) => {
  ...
  return (
    <div
      className="w-full h-1/2 flex flex-col justify-end p-5 md:p-10 items-start"
      style={{
    ...
    >
      <h1 className="text-3xl md:text-5xl font-bold text-zinc-100 flex items-end">
        {data.title || data.name || data.original_title || data.original_name}
      </h1>
      <p className="w-full md:w-1/2 mt-2 text-zinc-400 text-sm md:text-base">
        {data.overview
      ...
      <Link
        ...
        className="mt-3 p-2 bg-[#6556CD] rounded flex items-center font-sm font-semibold text-sm md:text-base"
      >
        <i className="ri-play-circle-fill text-xl md:text-2xl font-normal"></i>
        &nbsp;Watch Trailer
      </Link>
    </div>
  )
